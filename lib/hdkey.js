...
Object.defineProperty(HDKey.prototype, 'privateKey', {
  ...
  set: function (value) {
    ...
    assert(privateKeyVerify(value) === true, 'Invalid private key')
...
Object.defineProperty(HDKey.prototype, 'publicKey', {
  ...
  set: function (value) {
    ...
    assert(publicKeyVerify(value) === true, 'Invalid public key')
...
HDKey.prototype.verify = function (hash, signature) {
  if (hash.length !== 32) {
    throw new Error('message length is invalid')
  }
  if (signature.length !== 64) {
    throw new Error('signature length is invalid')
  }
  var sigObj = { r: signature.slice(0, 32), s: signature.slice(32, 64) }
  return verify(hash, sigObj, this.publicKey)
}
...
function privateKeyVerify (privateKey) {
  ...
  return privateKey.length === 32 && !bn.isZero() && bn.cmp(ecparams.n) < 0
}

function publicKeyCreate (privateKey, compressed) {
  privateKey = toBuffer(privateKey)
  var publicKeyObject = ec.keyFromPrivate(privateKey).getPublic(compressed)
  var publicKey = Buffer.from(publicKeyObject.encode('hex', compressed), 'hex')
  return publicKey
}
...
function publicKeyConvert (publicKey, compressed) {
  ...
  var publicKeyObject = ec.keyFromPublic(publicKey).getPublic(compressed)
  var result = Buffer.from(publicKeyObject.encode('hex', compressed), 'hex')
  return result
}

function privateKeyTweakAdd (privateKey, tweak) {
  var bn = new BN(tweak)
  if (bn.cmp(ecparams.n) >= 0) throw new Error('EC_PRIVATE_KEY_TWEAK_ADD_FAIL')
...
function publicKeyTweakAdd (publicKey, tweak, compressed) {
  ...
  tweak = new BN(tweak)
  if (tweak.cmp(ecparams.n) >= 0) throw new Error('EC_PUBLIC_KEY_TWEAK_ADD_FAIL')
  return Buffer.from(ecparams.g.mul(tweak).add(publicKeyObject).encode(true, compressed))
}

function sign (hash, privateKey) {
  var signature = ec.sign(hash, privateKey)
